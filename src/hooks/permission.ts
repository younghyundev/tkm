...
function main(): void {
  ...
  try {
    ...
    if (sessionId) {
      ...
      if (resolvedGen) {
        ...
        process.stderr.write(`tokenmon permission: no gen binding for session ${sessionId}, skipping\n`);
  ...
  const result = withLock(() => {
    ...
    writeConfig(config, generation); // permission_master may update max_party_size
  });
